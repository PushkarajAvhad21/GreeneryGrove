...
exports.downloadSalesByPlant = async (req, res) => {
    try {
        ...
        const pythonProcess = spawn('python', ['./analytics/getsalescsv.py', JSON.stringify(plantData)]);
        let result = '';
        ...
        pythonProcess.on('close', (code) => {
            if (code !== 0) {
                ...
                try {
                    ...
                    const csvPath = parsedResult.csv_path;
                    if (fs.existsSync(csvPath)) {
                        res.download(csvPath, 'sales_last_30_days.csv', (err) => {
                            if (err) {
